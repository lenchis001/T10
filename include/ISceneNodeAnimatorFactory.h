...
namespace scene
{
	...
	*/
	...
	public:
		...
		virtual boost::shared_ptr<ISceneNodeAnimator> createSceneNodeAnimator(ESCENE_NODE_ANIMATOR_TYPE type, boost::shared_ptr<ISceneNode> target) = 0;
		...
		\return Returns pointer to the new scene node animator or null if not successful. You need to
		drop this pointer after calling this, see IReferenceCounted::drop() for details. */
		virtual boost::shared_ptr<ISceneNodeAnimator> createSceneNodeAnimator(const c8* typeName, boost::shared_ptr<ISceneNode> target) = 0;
