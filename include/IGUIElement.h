...
public:
	...
	IGUIElement(EGUI_ELEMENT_TYPE type, boost::shared_ptr<IGUIEnvironment> environment, boost::shared_ptr<IGUIElement> parent,
		...
		Environment(environment), Type(type), _isNonSaveable(false)
	{
		#ifdef _DEBUG
	...
	*/
	...
	virtual bool isNonSaveable() const {
		return _isNonSaveable;
	}

	virtual void setIsNonSaveable(bool value) {
		_isNonSaveable = value;
	}
